...
> layer-down <id>`.
> **ОСТАЛОСЬ (для использования как продукт):** трансформа слоёв (PiP позиция/масштаб); сделать
> компоновщик ОСНОВНЫМ режимом (убрать флаг + старый SurfaceFilterRender-путь); **поворот/портрет
> (Bug 10) на матрицах композитора**; проверить реальную USB-камеру. ⚠️ НЕ делать дефолтом, пока не
> сверены ПОРТРЕТ и реальная USB (нужен Криник). Это следующий заход.
> 📌 git push всю ночь падал на auth (keychain) — **есть НЕзапушенные локальные коммиты**, запушить утром.
>
